...
export function getOutputFilePath(
  ...
  if (format === "es") {
    return resolve(outDir, `${safePackageName(getJsonFileInfo(packageJson).name)}.esm.js`);
  }
  return resolve(
    outDir,
...
export function getReactVersion({ dependencies, devDependencies }: PackageJson) {
  return (dependencies && dependencies.react) || (devDependencies && devDependencies.react);
}
