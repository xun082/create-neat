      ...
      CallExpression(path: any, file: any) {
        ...
        if (path.get("callee").isIdentifier({ name: "invariant" })) {
          ...
          const errorMessageQuasi = errorMsgLiteral
            .split("%s")
            .map((raw: any) => t.templateElement({ raw, cooked: String.raw({ raw } as any) }));

          // Import ReactError
          const reactErrorIdentifier = addDefault(path, resolveApp("/errors/ErrorDev.js"), {
          ...
          if (noMinify) {
            path.replaceWith(
              t.ifStatement(t.unaryExpression("!", condition), t.blockStatement([devThrow])),
            );
            return;
          }
          ...
          const reactErrorProdIdentfier = addDefault(path, resolveApp("/errors/ErrorProd.js"), {
