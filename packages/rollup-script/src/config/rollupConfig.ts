...
export async function createRollupConfig(
  ...
  const shouldMinify =
    options.minify !== undefined ? options.minify : options.env === "production" || isEsm;

  const formatString = ["esm", "cjs"].includes(options.format) ? "" : options.format;
  ...
  return {
    ...
    plugins: [
      ...
      resolve({
        mainFields: ["module", "main", options.target !== "node" ? "browser" : undefined].filter(
          Boolean,
        ) as string[],
      ...
      commonjs({
        ...
        include: options.format === "umd" ? /\/node_modules\// : /\/regenerator-runtime\//,
      }),
      json(),
      {
      ...
      typescript({
        ...
        tsconfigOverride: {
          compilerOptions: {
            ...
            ...(outputNumber > 0 ? { declaration: false, declarationMap: false } : {}),
          },
        },
